(() => {
    ...
    return `<main class="screen">
      ...
      <div class="list">
        <div class="next-job-block">
          <div class="job-row next-job-row" role="button" tabindex="0" onclick="nav('job')"><div class="job-time">9:00<span class="next">NEXT</span></div><div><div class="job-name">Sarah Chen</div><div class="job-desc">Tesla Model Y · Full detail</div></div><div class="job-price">$220<span class="chev">›</span></div></div>
          <button class="primary home-start" onclick="startJob()">Start job <span class="arrow-inline">→</span></button>
        </div>
  ...
  window.startJob = () => {
    state.started = true;
    nav('active');
    setTimeout(() => toast('Job started · 9:07 AM'), 70);
  };
  ...
  function job() {
    return `<main class="screen no-tabs">${topNav('Sarah Chen')}${stage(0)}
      ...
      <div class="sticky-action"><div class="sticky-kicker">Next step</div><button class="primary" onclick="startJob()">Start job <span class="arrow-inline">→</span></button></div>
    </main>`;
  }

  function beforeRecord() {
    if (!state.beforeShots) return '';
    return `<button class="record-line" onclick="toast('Before record: curb rash + rear bumper scuff')"><div><strong>Before record</strong><span>${state.beforeShots} photos · ${state.conditions || 2} conditions documented</span></div><div class="record-lock">Locked ›</div></button>`;
  }
  ...
  function active() {
    ...
    const hubSub = state.workStarted ? 'Keep shooting as you work. CoastOps organizes later.' : 'One walkaround. No tagging — timestamps lock these as pre-service.';
    ...
    if (!state.beforeShots) {
      ...
      nextAction = `<div class="sticky-action"><div class="sticky-kicker">Before record saved</div><button class="primary" onclick="beginWork()">Start work <span class="arrow-inline">→</span></button></div>`;
    ...
    return `<main class="screen no-tabs">${topNav(state.workStarted ? 'Active job' : 'Before walkaround')}${stage(phase)}
      <div class="job-status">${state.workStarted ? 'In progress · 9:13 AM' : 'Job started · 9:07 AM'}</div>
      <div class="workspace-title"><h1>Sarah Chen</h1><div class="sub">Tesla Model Y · Full detail</div></div>
      <div class="photo-hub" role="button" tabindex="0" onclick="openCamera('${state.workStarted ? 'work' : 'before'}')">
      ...
      ${suggestedIssue()}
      ${approvalRecord()}
      ${!state.workStarted && !state.beforeShots ? `<div class="inline-status">Capture condition before work starts. <strong>No categories to choose.</strong></div>` : ''}
  ...
  window.beginWork = () => {
    ...
    nav('active');
    setTimeout(() => toast('Work started · before record locked'), 70);
  };
  ...
  function camera() {
    ...
    const thumbs = Array.from({ length: Math.min(count, 6) }, () => '<div class="thumb"></div>').join('');
    const hint = cameraMode === 'before' ? 'Walk around once — no tagging needed' : 'Keep shooting — organization happens later';
  ...
  window.capture = () => {
    ...
    const n = document.getElementById('shotnum');
    if (n) n.textContent = cameraMode === 'before' ? state.beforeShots : state.workShots;
  };
  ...
  window.finishCapture = () => {
    ...
    if (cameraMode === 'before') {
      state.conditions = 2;
      nav('active');
      setTimeout(() => toast(`${state.beforeShots} before photos locked · 2 conditions documented`), 80);
    } else {
      nav('active');
  ...
  function customer() {
    return `<main class="customer-shell"><div class="customer-brand"><div class="name">COASTAL DETAIL</div><div class="meta">412 Bay Ave · Capitola, CA</div></div>
  ...
  function complete() {
    ...
    return `<main class="screen no-tabs">${topNav('Finish job')}${stage(3)}
      ...
      <div class="auto-note"><div class="auto-mark"></div><div><strong>Best proof selected automatically</strong><span>Original timestamps, before record and approval remain attached to the job.</span></div></div>
  ...
  function pay() {
    const total = 220 + (state.approval === 'approved' ? state.addOn.price : 0);
    const totalShots = state.beforeShots + state.workShots;
    return `<main class="customer-shell"><div class="customer-brand final-brand"><div class="name">COASTAL DETAIL</div><div class="meta">Job completed · 10:42 AM</div><button class="return-home" onclick="nav('today')">Return to home</button></div>
